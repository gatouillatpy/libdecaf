...
#ifndef __DECAF_ED168_HXX__
#define __DECAF_ED168_HXX__ 1
/*
...
/** A public key for crypto over E-168 */
template<> struct EdDSA<E168> {
 ...
 */
#if DECAF_EDDSA_168_SUPPORTS_CONTEXTLESS_SIGS
static inline const Block NO_CONTEXT() { return Block(DECAF_ED168_NO_CONTEXT,0); }
#else
...
private:
    ...
    void init() /*throw(LengthException)*/ {
        ...
        decaf_ed168_prehash_init((decaf_shake256_ctx_s *)wrapped);
    }
    /** @endcond */
...
public:
    ...
    inline SecureBuffer sign_prehashed ( const Prehash &ph ) const /*throw(std::bad_alloc)*/ {
        SecureBuffer out(CRTP::SIG_BYTES);
        decaf_ed168_sign_prehash (
            out.data(),
            ((const CRTP*)this)->priv_.data(),
            ((const CRTP*)this)->pub_.data(),
            (const decaf_ed168_prehash_ctx_s*)ph.wrapped,
            ph.context_.data(),
            ph.context_.size()
        );
        return out;
...
/** @endcond */
    ...
    FixedArrayBuffer<DECAF_EDDSA_168_PRIVATE_BYTES> priv_;
    
    /** The post-expansion public key. */
    FixedArrayBuffer<DECAF_EDDSA_168_PUBLIC_BYTES> pub_;
...
public:
    ...
    /** Signature size. */
    static const size_t SIG_BYTES = DECAF_EDDSA_168_SIGNATURE_BYTES;
    
    /** Serialization size. */
    static const size_t SER_BYTES = DECAF_EDDSA_168_PRIVATE_BYTES;
    ...
    inline explicit PrivateKeyBase(Rng &r) DECAF_NOEXCEPT : priv_(r) {
        decaf_ed168_derive_public_key(pub_.data(), priv_.data());
    }
    ...
    inline SecureBuffer convert_to_x() const {
        SecureBuffer out(DECAF_X168_PRIVATE_BYTES);
        decaf_ed168_convert_private_key_to_x168(out.data(), priv_.data());
        return out;
    }
...
public:
    ...
    inline decaf_error_t DECAF_WARN_UNUSED verify_noexcept (
        const FixedBlock<DECAF_EDDSA_168_SIGNATURE_BYTES> &sig,
        const Block &message,
        const Block &context = NO_CONTEXT()
...
public:
    ...
    inline decaf_error_t DECAF_WARN_UNUSED verify_prehashed_noexcept (
        ...
        return decaf_ed168_verify_prehash (
            sig.data(),
            ((const CRTP*)this)->pub_.data(),
            (const decaf_ed168_prehash_ctx_s*)ph.wrapped,
            ph.context_.data(),
    ...
    inline void verify_prehashed (
        ...
        if (DECAF_SUCCESS != decaf_ed168_verify_prehash (
            sig.data(),
            ((const CRTP*)this)->pub_.data(),
            (const decaf_ed168_prehash_ctx_s*)ph.wrapped,
            ph.context_.data(),
...
public:
    ...
    inline SecureBuffer convert_to_x() const {
        SecureBuffer out(DECAF_X168_PRIVATE_BYTES);
        decaf_ed168_convert_public_key_to_x168(out.data(), pub_.data());
        return out;
    }
}; /* class PublicKey */
