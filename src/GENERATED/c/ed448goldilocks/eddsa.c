...
static void field_print3(const uint8_t *ser) {
    int i;
    fprintf(stderr, "0x");
    for (i = DECAF_EDDSA_448_PRIVATE_BYTES - 1; i >= 0; i--) {
        fprintf(stderr, "%02x", ser[i]);
    }
}
...
#if NO_CONTEXT
const uint8_t DECAF_ED448_NO_CONTEXT_POINTS_HERE = 0;
const uint8_t * const DECAF_ED448_NO_CONTEXT = &DECAF_ED448_NO_CONTEXT_POINTS_HERE;
#endif
